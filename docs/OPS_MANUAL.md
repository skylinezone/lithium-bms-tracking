...
| 优先级 | 来源 | 说明 |
|--------|------|------|
| **P1 必须** | 论文官网/期刊官方 Figure | Nature/ScienceDirect/Springer 等原始 figure 图片 |
| P2 参考 | 学术图片库 | Science/AAAS、ACS、Pubs 等官方图库 |
| P3 备选 | AI 生成描述性图片 | 仅前两者均无法获取时使用，且必须与论文内容高度相关 |

**图片文件命名**：按论文主题命名，禁止不同方向论文共用同一张图片。
