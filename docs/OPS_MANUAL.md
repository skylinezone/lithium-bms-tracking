...
| 功能 | 说明 |
|------|------|
| 历史存储 | 每次提交反馈自动保存到 localStorage，换浏览器/设备后历史不丢失 |
| 日期字段 | 每条反馈自动标记 `YYYY-MM-DD` 提交日期 |
| 精选标记 | 可将任意反馈标记为"精选"（紫色高亮），方便置顶重要建议 |
| 排序方式 | 5种排序：📅最新在前 / 📅最早在前 / ⭐高分优先 / ⭐低分优先 / ✨精选优先 |
| 日期筛选 | 下拉菜单自动列出所有有反馈的日期，支持精确按日期过滤 |
| 评分筛选 | ⭐≥8 / 👍5-7 / 💡<5 三个级别快速筛选 |
| 精选筛选 | "✨仅精选"开关，一键只看精选建议 |
| 关键词搜索 | 搜索建议内容 |
| 单条删除 | 每条反馈支持删除（二次确认） |
| 批量清空 | 清空全部历史（带风险提示 + 导出备份提醒） |
| 精选统计 | 统计卡片新增"✨精选"数量显示 |

**数据存储架构：**
```
提交反馈 → localStorage（立即生效）
          ↘ GitHub 备份（通过 git push 同步 feedback_history.json）
换设备/浏览器 → 访问 GitHub Pages → 手册自动加载 GitHub 历史数据
```

**⚠️ 重要提醒：**
- 反馈数据保存在本地浏览器，换浏览器后需要手动从 GitHub 拉取历史
- 如果需要跨浏览器保留历史，可以在 A 浏览器导出 JSON，再到 B 浏览器导入
- `feedback_history.json` 文件会随每次 git push 同步到 GitHub

### 🚨 问题修复（2026-03-22）

| # | 问题 | 根因 | 修复 |
|---|------|------|------|
| 1 | 反馈手册只有当天数据，换日期后历史丢失 | 只用 localStorage，无持久化 | 每次提交同时写入 `feedback_history.json`，git push 同步到 GitHub |
| 2 | 没有日期字段，无法按日期筛选 | `FeedbackItem` 接口缺少 `date` 字段 | 新增 `date: YYYY-MM-DD` 字段 |
| 3 | 无法标记精选建议 | 无 `featured` 字段和 UI 操作入口 | 新增 `featured: boolean` 字段 + 手册内切换按钮 |
| 4 | 不知道历史反馈从哪天开始 | 无排序功能 | 新增5种排序（按日期/评分/精选） |
| 5 | 无法按日期范围查看反馈 | 无日期筛选器 | 新增日期下拉筛选（自动列出所有有数据的日期） |
| 6 | 反馈导出 CSV 缺少日期和精选列 | 旧导出格式不完整 | 新导出格式含：ID、日期、评分、精选、建议内容、提交时间、页面上下文 |
| 7 | 手册页面 TypeScript 类型错误导致构建失败 | `confirmClear` 类型声明为 `boolean` 但实际用于存储 `string \| false` | 修正类型声明为 `string \| false` |
...
2. pnpm build
     ↓
3. cp dist/index.html docs/
     ↓
4. cp dist/assets/*.css docs/assets/
     ↓
5. cp dist/assets/*.js docs/assets/
     ↓
6. git add -A && git commit -m "描述"
     ↓
7. git push origin main
     ↓
8. GitHub Pages 自动从 /docs 构建（3-5分钟生效）
     ↓
9. 访问 https://skylinezone.github.io/lithium-bms-tracking/
```

**永远不变的地址：**
- 🌐 **GitHub Pages（生产环境）：** `https://skylinezone.github.io/lithium-bms-tracking/`
- 🔧 **本地预览：** `cd lithium-tracking && pnpm dev`（启动开发服务器）

**`deploy` 工具使用场景（仅限临时预览）：**
- 需要临时分享给外部人员预览新功能时
- 需要在正式 push 前验证构建结果时
- 临时演示/测试用途，**绝不**用于生产环境
